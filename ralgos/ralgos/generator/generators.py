...
class normal(Generator):
    def __init__(self,mean=0,std=1,m=100):
        ...
        super().__init__(sampling_function=gen.sample, k="i",type="normal",lower_bound="minf",upper_bound="pinf")
        self.mean = mean
        self.std = std
...
class uniform(Generator):
    def __init__(self,m=100):
        ...
        super().__init__(sampling_function=gen.sample, mean=m//2,k="i",type="uniform",lower_bound=0,upper_bound=m)


class uuniform(Generator):
    def __init__(self,p=104729,m=100):
        gen = UUniformGenerator(p,m)
        super().__init__(sampling_function=gen.sample, mean=m//2,k="u",type="uuniform",lower_bound=0,upper_bound=m)
